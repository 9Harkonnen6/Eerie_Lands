...
from perlinMapGenerator import *
import random
...
tileset_dictionary_A = {"grass": [grass, grass_1, grass_2],
                      "water": [water, water_1, water_2],
                      "dirt": [dirt, dirt_1, dirt_2],
                      "stone": [concrete, concrete_1, concrete_2],
                      "tree": [tree, tree_1, tree_2, tree_3, tree_4],
                      "bush": [grass_addons_4, grass_addons_5, grass_addons_6, bush, bush_2, bush_3]
...
while True:
    ...
    for row in pic:
        ...
        for tile in row:
            #water
            if tile < -0.17:
                display.blit(random.choice(tileset_dictionary_A['water']) , (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
            #grass
            elif tile > 0:
                display.blit((random.choice(tileset_dictionary_A['grass'])), (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
                if random.randint(0, 4) ==1:
                    display.blit(random.choice(tileset_dictionary_A['tree']) , (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
                elif random.randint(0,5) == 1:
                    display.blit(random.choice(tileset_dictionary_A['bush']) , (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
                else:
                    pass
            #dirt
            elif tile > -0.17 and tile < 0:
                display.blit(random.choice(tileset_dictionary_A['dirt']) , (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
            else:
                display.blit(random.choice(tileset_dictionary_A['dirt']) , (x * TILE_SIZE + scroll[0], y * TILE_SIZE + scroll[1]))
            x += 1
        y += 1
